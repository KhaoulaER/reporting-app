...
export class AuditHeadComponent implements OnInit {
  ...
  handleOpenAuditClick(){
    this.auditValidationService.updateValidateButtonState(false);
    this.auditValidationService.auditValidation(this.normeAdopte.id, false).subscribe({
      next: (response) => {
        console.log('Audit reinitialise: ',response);
        this.isValidateButtonClicked = false;
        this.clickValidate.emit(response);
        this.messageService.add({ severity: 'success', summary: 'Success', detail: 'Audit reinitialise !' });
      }
    })
  }
